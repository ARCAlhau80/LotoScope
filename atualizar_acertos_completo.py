...
# -*- coding: utf-8 -*-
"""
🔄 ATUALIZADOR COMPLETO DE ACERTOS - LOTOFÁCIL 20 NÚMEROS
=========================================================
Processa TODOS os tipos de acertos (11, 12, 13, 14, 15) com lógica incremental inteligente.
...
MODOS DE EXECUÇÃO:
  python atualizar_acertos_completo.py           # Incremental automático
  python atualizar_acertos_completo.py --full    # Forçar processamento completo
  python atualizar_acertos_completo.py --desde X # Processar a partir do concurso X
...
class AtualizadorAcertosCompleto:
    """
    Classe responsável pela atualização completa de acertos
    com suporte a processamento incremental inteligente.
    """
    
    # Configurações de processamento
    BATCH_SIZE = 100000  # Combinações por lote (ajuste conforme memória)
    NIVEIS_ACERTOS = [11, 12, 13, 14, 15]  # Todos os níveis premiados
    ...
    def verificar_estrutura_tabela(self) -> bool:
        """
        Verifica se todas as colunas necessárias existem na tabela.
        """
        ...
        AND COLUMN_NAME IN ({})
        """.format(','.join(f"'{c}'" for c in colunas_necessarias))
        ...
        if colunas_faltantes:
            ...
            print("💡 Execute primeiro: adicionar_colunas_acertos.sql E adicionar_colunas_ultimo_acerto.sql")
            return False
    ...
    def obter_menor_concurso_atualizado(self) -> int:
        """
        Obtém o menor valor de UltimoConcursoAtualizado na tabela.
        Isso determina até onde já foi processado globalmente.
        """
        query = """
    ...
    def obter_concursos_pendentes(self, desde_concurso: int) -> List[Dict]:
        ...
        Args:
            desde_concurso: Concurso a partir do qual buscar (exclusivo)
        
        Returns:
            Lista de dicionários com {concurso, numeros}
        """
        query = f"""
        SELECT 
            Concurso,
    ...
    def calcular_acertos(self, numeros_combinacao: List[int], numeros_sorteio: List[int]) -> int:
        """
        Calcula quantos números da combinação coincidem com o sorteio.
        
        Args:
            numeros_combinacao: Lista de 20 números da combinação
            numeros_sorteio: Lista de 15 números sorteados
        
        Returns:
            Quantidade de acertos (0-15)
        """
        return len(set(numeros_combinacao) & set(numeros_sorteio))
    
    def processar_concurso_sql(self, concurso: int, numeros: List[int]) -> Dict[int, int]:
        """
        Processa um concurso usando SQL puro (mais eficiente para grandes volumes).
        Atualiza contagens E campos de último acerto em uma única transação.
        
        Args:
            concurso: Número do concurso
            numeros: Lista de 15 números sorteados
        
        Returns:
            Dicionário com quantidade de combinações atualizadas por nível
        """
        numeros_str = ','.join(map(str, numeros))
        resultados = {}
        
        # SQL para contar acertos de uma combinação de 20 números
        ...
        for nivel in self.NIVEIS_ACERTOS:
            # UPDATE: incrementa contagem E atualiza último concurso
            sql_update = f"""
            ...
            try:
                # Conta quantas linhas serão afetadas primeiro
                sql_count = f"""
                SELECT COUNT(*) as qtd
                FROM COMBINACOES_LOTOFACIL20_COMPLETO 
                WHERE {sql_contar_acertos} = {nivel}
    ...
    def atualizar_controle_concurso(self, concurso: int):
        """
        Atualiza o campo UltimoConcursoAtualizado para TODAS as combinações.
        
        Args:
            concurso: Número do último concurso processado
        """
        sql_update = f"""
        UPDATE COMBINACOES_LOTOFACIL20_COMPLETO 
    ...
    def processar_lote_concursos(self, concursos: List[Dict], mostrar_progresso: bool = True) -> bool:
        ...
        Args:
            concursos: Lista de concursos a processar
            mostrar_progresso: Se deve mostrar progresso detalhado
    ...
    def executar_atualizacao_incremental(self) -> bool:
        ...
        Processa apenas concursos novos (após UltimoConcursoAtualizado).
        
        Returns:
            True se executou com sucesso
        """
        print("\n" + "=" * 70)
        print("🔄 ATUALIZAÇÃO INCREMENTAL DE ACERTOS - LOTOFÁCIL 20")
        print("=" * 70)
    ...
    def executar_atualizacao_completa(self, desde_concurso: Optional[int] = None, auto_confirm: bool = False) -> bool:
        """
        Executa atualização completa (zera e recalcula tudo ou desde um concurso específico).
        
        Args:
            desde_concurso: Se especificado, processa a partir deste concurso.
                           Se None, processa desde o primeiro concurso.
            auto_confirm: Se True, não pede confirmação para reset.
        
        Returns:
            True se executou com sucesso
        """
        print("\n" + "=" * 70)
        print("🔄 ATUALIZAÇÃO COMPLETA DE ACERTOS - LOTOFÁCIL 20")
        print("=" * 70)
        print(f"📅 Data/Hora: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}")
        ...
        if desde_concurso is None:
            ...
            # Zerar contagens se for completo desde o início
        ...
        # Processar em lotes menores para mostrar progresso
        LOTE = 50
    ...
    def gerar_relatorio_final(self):
        ...
        # Consultar estatísticas finais da tabela
        query_stats = """
        SELECT 
...
def main():
    """Função principal com tratamento de argumentos."""
    parser = argparse.ArgumentParser(
        description='Atualizador completo de acertos - Lotofácil 20 números',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemplos de uso:
  python atualizar_acertos_completo.py           # Incremental automático
  python atualizar_acertos_completo.py --full    # Reprocessar tudo
  python atualizar_acertos_completo.py --desde 3600  # A partir do concurso 3600
        """
    )
    ...
    parser.add_argument(
        '--full', 
        action='store_true',
        help='Força processamento completo desde o início'
    )
    
    parser.add_argument(
        '--desde', 
        type=int,
        help='Processa a partir de um concurso específico'
    )
    ...
    atualizador = AtualizadorAcertosCompleto()
    ...
    try:
        if args.full:
            ...
            # Modo desde concurso específico
