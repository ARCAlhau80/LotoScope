...
"""
🔺 SISTEMA PIRÂMIDE INVERTIDA DINÂMICA
Sistema avançado que analisa movimentação de números entre faixas de acertos
usando análise temporal e rede neural para predições de transições.

Módulos:
1. Analisador da Pirâmide Atual - Identifica configuração atual das faixas
2. Monitor de Sequências - Rastreia qual número/sequência domina a janela
3. Detector de Movimentações - Identifica subidas/descidas entre faixas
4. Preditor de Próxima Faixa - Prevê onde cada número estará no próximo ciclo
5. Gerador Baseado em Transições - Usa movimentações para gerar combinações
...
class PiramideInvertidaDinamica:
        ...
        try:
            ...
            for i in range(num_amostras):
                ...
                combinacao_piramide = self._gerar_combinacao_piramide(
                    piramide_atual, predicoes, sequencias, 15
                )
    ...
    def classificar_numero_em_faixa(self, qtd_sorteados: int) -> str:
    ...
    def monitorar_sequencias(self, ciclos_analise: int = 10) -> Dict:
        ...
        for numero in range(1, 26):
            sequencia_atual = {'tamanho': 0, 'ciclos': []}
    ...
    def detectar_movimentacoes(self, ciclos_comparacao: int = 5) -> Dict:
        ...
        for i in range(1, len(ciclos_analise)):
    ...
    def _predizer_empiricamente(self, faixa_atual: str, prob_sair: float) -> str:
        ...
        if faixa_atual == '0_acertos':
            ...
            if np.random.random() < prob_sair:
                return np.random.choice(['1_acerto', '2_acertos'], p=[0.7, 0.3])  # Variação na saída
            else:
                return '0_acertos'
            ...
            if np.random.random() < prob_sair:
                return np.random.choice(['2_acertos', '3_acertos'], p=[0.6, 0.4])  # Mais para 3_acertos
            else:
                return np.random.choice(['0_acertos', '1_acerto'], p=[0.6, 0.4])  # Alguns voltam
        
        elif faixa_atual == '2_acertos':
            # 40% chance de sair (MENOS que antes para reduzir concentração)
            if np.random.random() < prob_sair:
                return np.random.choice(['3_acertos', '1_acerto', '0_acertos'], p=[0.5, 0.3, 0.2])  # Distribuição variada
            else:
            ...
            if np.random.random() < prob_sair:
                return np.random.choice(['4_ou_mais', '2_acertos', '1_acerto'], p=[0.4, 0.4, 0.2])  # Distribui melhor
            else:
            ...
            if np.random.random() < prob_sair:
                return np.random.choice(['3_acertos', '2_acertos', '1_acerto'], p=[0.4, 0.4, 0.2])  # Distribui melhor a descida
            else:
    ...
    def gerar_baseado_transicoes(self, qtd_numeros: int = 15, quantidade: int = 10) -> List[List[int]]:
        ...
        for tentativa in range(quantidade * 3):  # Máximo de tentativas
            if len(combinacoes) >= quantidade:
                break
            
            combinacao = self._gerar_combinacao_piramide(
                piramide_atual, predicoes, sequencias, qtd_numeros
            )
    ...
    def _gerar_combinacao_piramide(self, piramide: Dict, predicoes: Dict, 
        ...
        while tentativas < max_tentativas:
            ...
            for numero in piramide.get('0_acertos', []):
