...
def gerar_diversificado_enxuto(sorteios, idx_atual, n_jogos=5, ciclo_dados=None):
    """
    Versao enxuta: foco em minimizar intersecao entre jogos e cobertura eficiente.
    Recomendado: 5 jogos. Permite 1 a N.
    """
    if n_jogos < 1:
        n_jogos = 1
    if n_jogos > 50:
        n_jogos = 50  # limite de seguranca

    ultimos = [sorteios[j][1] for j in range(max(0, idx_atual - 5), idx_atual)]
    jogos = []

    estrategias = [
        lambda: gerar_atraso(ultimos[-5:]),
        lambda: gerar_hot(ultimos[-3:], n_hot=random.randint(7, 9)),
        lambda: gerar_persistencia_categoria(ultimos[-2:], categoria=random.choice(['pares', 'impares', 'primos'])),
        lambda: gerar_aleatorio(),
        lambda: gerar_ciclo(ciclo_dados) if ciclo_dados else gerar_atraso(ultimos[-5:]),
    ]

    max_tentativas = n_jogos * 300
    tentativa = 0

    while len(jogos) < n_jogos and tentativa < max_tentativas:
        tentativa += 1
        # Escolhe estrategia baseada na posicao, mas permite repeticao se necessario
        estrategia_idx = len(jogos) % len(estrategias)
        jogo = estrategias[estrategia_idx]()

        if jogo in jogos:
            continue

        # Penaliza jogos muito similares: intersecao media com os existentes > 9
        if jogos:
            intersecoes = [len(set(jogo) & set(ex)) for ex in jogos]
            if mean(intersecoes) > 9:
                continue

        jogos.append(jogo)

    # Fallback: gera aleatorios unicos se necessario
    while len(jogos) < n_jogos:
        jogo = gerar_aleatorio()
        if jogo not in jogos:
            jogos.append(jogo)

    # Garante cobertura: insere numeros faltantes se n_jogos >= 3
    if n_jogos >= 3:
        cobertura = set(n for j in jogos for n in j)
        faltando = sorted(set(range(1, 26)) - cobertura)
        for num_faltando in faltando:
            # Encontra o jogo com maior intersecao media e troca um numero por num_faltando
            melhor_idx = 0
            melhor_score = -1
            for idx, jogo in enumerate(jogos):
                outras = [set(jogos[i]) for i in range(len(jogos)) if i != idx]
                if outras:
                    score = mean(len(set(jogo) & o) for o in outras)
                else:
                    score = 0
                if score > melhor_score:
                    melhor_score = score
                    melhor_idx = idx
            jogo = jogos[melhor_idx]
            for pos in range(15):
                novo_jogo = jogo[:pos] + jogo[pos+1:] + [num_faltando]
                novo_jogo = sorted(novo_jogo)
                if novo_jogo not in jogos and len(set(novo_jogo)) == 15:
                    jogos[melhor_idx] = novo_jogo
                    break
...
def backtest(sorteios, concurso_ciclo, n_jogos=5):
    resultados = []
    for i in range(5, len(sorteios)):
        ...
        jogos = gerar_diversificado_enxuto(sorteios, i, n_jogos=n_jogos, ciclo_dados=ciclo_dados)
        acertos = [len(set(j) & set(resultado_atual)) for j in jogos]
...
def main():
    ...
    for n in [1, 3, 5, 10, 20]:
        print(f"\n=== BACKTEST COM {n} JOGO(S) ===")
        res = backtest(sorteios, concurso_ciclo, n_jogos=n)
        maximos = [r['max'] for r in res]
        c = Counter(maximos)
        total = len(maximos)
        print(f"  Media do maximo acerto: {mean(maximos):.2f}")
        print(f"  Distribuicao: {dict(sorted(c.items()))}")
        print(f"  % com 11+: {sum(1 for r in res if r['max'] >= 11)/total*100:.2f}%")
        print(f"  % com 12+: {sum(1 for r in res if r['max'] >= 12)/total*100:.2f}%")
        print(f"  Cobertura media: {mean([r['cobertura'] for r in res]):.2f}")

    # Gerar para proximo concurso
    print("\n=== GERACAO PARA PROXIMO CONCURSO ===")
    ciclo_info = concurso_ciclo.get(sorteios[-1][0])
    ciclo_dados = ciclo_info[1] if ciclo_info else None
    for n in [1, 3, 5, 10]:
        print(f"\n{n} jogo(s):")
        jogos = gerar_diversificado_enxuto(sorteios, len(sorteios), n_jogos=n, ciclo_dados=ciclo_dados)
        for idx, jogo in enumerate(jogos, 1):
            print(f"  Jogo {idx}: {' '.join(f'{x:02d}' for x in jogo)}")
        print(f"  Cobertura: {len(set(x for j in jogos for x in j))}/25")
